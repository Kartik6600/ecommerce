...
const Collection = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row pt-4 md:pt-10 border-t border-[#191973] h-full min-h-screen">
      <div
        className={`fixed md:static top-0 left-0 bg-zinc-100 md:bg-transparent z-30 md:z-auto w-3/4 md:w-64 h-full md:h-auto overflow-y-auto md:overflow-visible transition-transform duration-300 ease-in-out ${
          showFilter ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <div className="p-4 md:p-0 md:pr-4">
          <div className="flex justify-end md:hidden mb-4">
            <button onClick={() => setShowFilter(false)} className="text-[#191973] text-2xl">
              <FaTimes />
            </button>
          </div>
          <p className="my-2 text-xl font-semibold text-[#191973]">FILTERS</p>
          <div className="mb-4 space-y-4">
            <FilterGroup title="CATEGORIES" options={categoryOptions} toggle={toggleCategory} />
            <FilterGroup title="TYPE" options={subCategoryOptions} toggle={toggleSubCategory} />
            <FilterGroup title="SIZE" options={sizeOptions} toggle={toggleSize} />
          </div>
        </div>
      </div>
    ...
    <Title text1="ALL" text2="COLLECTIONS" />
    <div className="relative group text-[#191973] self-start sm:self-auto">
      <button className="bg-zinc-100 border-2 border-[#191973] rounded-md px-3 py-1 flex items-center gap-2 text-sm font-medium w-40 justify-between">
        <span className="flex items-center gap-2">Sort By</span>
        <FaChevronRight className="group-hover:rotate-90 transition-transform" />
      </button>
      <div className="absolute z-10 hidden group-hover:flex flex-col mt-1 w-40 bg-white border-2 border-[#191973] rounded-md shadow-md text-sm font-medium">
        {["relavent", "low-high", "high-low"].map((type) => (
...
const FilterGroup = ({ title, options, toggle }) => (
  <div className="bg-zinc-100 border-2 border-[#191973] rounded-md p-4">
    <p className="mb-3 text-sm font-medium text-[#191973]">{title}</p>
    <div className="flex flex-col gap-2 text-sm font-light text-[#101049]">
      {options.map((option) => (
